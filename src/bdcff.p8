; Boulderdash Common File Format parsing
...
bdcff {
    ...
    sub parse_caveset() -> bool {
        ...
        if num_caves>0 {
            ...
            repeat {
                ...
                uword argptr = 0
                ubyte equalsIndex
                equalsIndex, void = strings.find(line, '=')
                if_cs {
                    argptr = line+equalsIndex+1
                    line[equalsIndex] = 0
                }
                if line=="Name"
                    void strings.copy(argptr, caveset_name)
                else if line=="Author"
                    void strings.copy(argptr, caveset_author)
                else if line=="Levels"
    ...
    sub parse_cave(ubyte level, ubyte difficulty) {
        ...
        for cx16.r0L in 0 to len(rand_seeds)-1 {
            rand_seeds[cx16.r0L] = 0
            cave_times[cx16.r0L] = 0
            diamonds_needed[cx16.r0L] = 0
        }
            ...
            when read_state {
                READ_CAVE -> {
                    if lineptr=="[map]" {
                        ...
                        argptr = 0
                        equalsIndex,void = strings.find(lineptr, '=')
                        if_cs {
                        ...
                        if lineptr=="Name" {
                            void strings.copy(argptr, cave.name)
                        }
                        else if lineptr=="Description" {
                            void strings.copy(argptr, cave.description)
                        }
...
;                        }
                            ...
                            numberStr[5] = 0
                            equalsIndex,void = strings.find(numberStr, '.')
                            if_cs {
                                ...
                                if strings.startswith(numberStr, "0.0")
                                    cave.slime_permeability = %11111111
                                else if strings.startswith(numberStr, "0.125")
                                    cave.slime_permeability = %11111110
                                else if strings.startswith(numberStr, "0.25")
                                    cave.slime_permeability = %11111100
                                else if strings.startswith(numberStr, "0.375")
                                    cave.slime_permeability = %11111000
                                else if strings.startswith(numberStr, "0.5")
                                    cave.slime_permeability = %11110000
                                else if strings.startswith(numberStr, "0.625")
                                    cave.slime_permeability = %11100000
                                else if strings.startswith(numberStr, "0.75")
                                    cave.slime_permeability = %11000000
                                else if strings.startswith(numberStr, "0.875")
                                    cave.slime_permeability = %10000000
                                else if strings.startswith(numberStr, "1.0")
            ...
            for cx16.r0L in 0 to MAX_WORDS-1 {
                words[cx16.r0L] = 0
                arg_bytes[cx16.r0L] = 0
            }
            ...
            sub parse_object(str name) -> uword {
                ...
                for cx16.r2L in 0 to len(names)-1 {
                    if strings.compare(name, names[cx16.r2L])==0
            ...
            sub split_words() {
                ...
                while @(argptr)!=0 and word_idx<len(words) {
                    ...
                    word_idx++
                    cx16.r0L,void = strings.find(argptr, ' ')
                    if_cc
                        return
                    argptr += cx16.r0L
                    @(argptr) = 0
